function DrawRasterfromSUA_CategorizationOK(sua_id, cond, times, EpochNum, SDF_MAX, monkey)
...
thisTitle = [tit ', Max Hz in Y = ' num2str(SDF_MAX)];
flag = 0;
...
%               figure; plot(samp_points,this_trial_sdf);
...
%               SD = SD  *  1000;
%               
%               x = randsample(spike_times, numel(spike_times));
              SD = permuteSD(spike_times,samp_points,[]);
              SD_permute = permuteSD(spike_times,samp_points,'Permute');

%               SD_permute = 0;
%               figure;
%               hold on;
%               plot(winTimes, SD, spike_times, 1, 'ok')
...
%             set(gca, 'XTick', pre(task):x_tick_span:post(task));        %  reset tick spacing
        ...
        if   trialcounter(8) == trial_max 
               text(-70, -125, '1st Stim', 'FontSize', 12);%,'Rotation',90); 
               text(580, -125, 'Begin Delay', 'FontSize', 12);%,'Rotation',90); 
               text(1530, -125, 'Target ON', 'FontSize', 12);%,'Rotation',90); 
               text(post(task)-420, -160, '500 ms/div', 'FontSize', 14);%,'Rotation',90); 
        end
    end
%print(tit,'-dpdf','-painters','-bestfit',  '-r300');% -fillpage')
end
