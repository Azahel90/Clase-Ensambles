function [coef,resid,ypred,mse] = curvefitting6functions2019_V2(x,y,model)
%%HM 3/18/2015
...
switch(model)
    case(1) %linear
        ...
        [m,b,mse,r2] = logfit(x,y,'linear');%[slope intercept]
        ypred = (b) + x*m;
    ...
    case(2) %power
        ...
        [m,b,mse,r2] = logfit(x,y,'loglog');%[slope intercept]
        ypred = (10^b)*x.^(m);
        resid = y - ypred;
        coef(1) = m;
        coef(2) = b;
    ...
    case(3) %expontial
        ...
        [m,b,mse,r2] = logfit(x,y,'logy');%[slope intercept]
         ypred = (10^b)*(10^m).^x;
         resid = y - ypred;
        coef(1) = m;
        coef(2) = b;
    ...
    case(4) %logarithmic
        %Logatithmic Currve Fit: y = b + mlnx
         [m,b,mse,r2] = logfit(x,y,'logx');%[slope intercept]
         ypred = (b) + m*log10(x);
         resid = y - ypred;
        coef(1) = m;
        coef(2) = b;
