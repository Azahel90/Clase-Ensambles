...
%getArgumentValue() Is used within a function to get the value following an argument 
%                   in an 'argument',[value] pair within VARARGIN.
%
%   value = getArgumentValue('argument',defaultvalue,varargin{:})              % Use default value if ARGUMENT is not found.
%   value = getArgumentValue('argument',defaultvalue,varargin{:},'warningoff') % Use defalut value, but don't issue a warning. 
%   []    = getArgumentValue('argument',[],varargin)                           % No default value is necessary.
%   []    = getArgumentValue('argument','msg:warning msg',varargin{:})     % No default value is necessary, but display a msg.
%   []    = getArgumentValue('argument','errorIfEmpty',varargin{:})        % The argument-value pair must be provided.
%
